...
import { CPF_MÁSCARA } from "../../utilitarios/máscaras";
import {
  MostrarMensagemErro,
  checarListaVazia,
  validarCampoEmail,
...
import { serviçoVerificarCpfExistente } from "../../servicos/serviços-usuário";
export default function CadastrarUsuário() {
  ...
  function validarCamposAdministrar() {
    const { email, senha, confirmação, questao, resposta } = dados;
  ...
  function validarCamposCadastrar() {
    const { perfil, cpf, nome, questao, resposta, senha, confirmação, email } =
      dados;
    console.log(
  ...
  async function validarConfirmarCriação() {
    ...
    console.log("Campos válidos:", camposVálidos); // Adicione este log
    if (camposVálidos) {
      let response;
      try {
  ...
  function confirmarOperação(operação) {
    console.log("Confirmando operação:", operação); // Adicione este log
      ...
      >
        ...
        <div className={estilizarDivCampo()}>
          ...
          <InputText
            ...
            tooltip={
              usuárioLogado?.token &&
              "Se a resposta não for informada: a alteração de questao será ignorada."
            }
          />
